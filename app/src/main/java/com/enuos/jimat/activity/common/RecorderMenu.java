				...
				switch (action) {
					...
					case MotionEvent.ACTION_UP:
						if (!mReady) {
							...
							/*Dialog显示时间短*/
							mDialogManager.dismissDialog();
							mRecordText.setText(R.string.my_ease_record_menu_too_short);
		...
		if (mCurState != state) {
			...
			switch (state) {
				case STATE_NORMAL:
					...
					mRecordText.setText(R.string.my_button_pushtotalk);
				...
				case STATE_RECORDING:
					if (!isHasRecorderPermission) {
						...
						if (!hasPermission) {
							reset();
							Toast.makeText(getContext(), R.string.my_Recording_without_permission, Toast.LENGTH_SHORT).show();
							return;
						}
					}
					isHasRecorderPermission = true;
					setBackgroundResource(R.drawable.hd_btn_recorder_recording);
					mRecordText.setText(R.string.my_recording_description);
					mRecordBtn.setBackgroundResource(R.drawable.hd_record_menu_mic_recording);
					//mDialogManager.recording();
					break;
				case STATE_WANT_TO_CANCEL:
					setBackgroundResource(R.drawable.hd_btn_recorder_recording);
					mRecordText.setText(R.string.my_release_to_cancel);
