...
import com.enuos.jimat.activity.msg.UploadRecordAdapter;
...
public class UploadRecordActivity extends BaseActivity {
    ...
    @BindView(R.id.mine_upload_record_back)
    ImageView mBack;
    @BindView(R.id.mine_upload_record_rv)
    PullToRefreshRecyclerView pullToRefreshRecyclerView;
    @BindView(R.id.mine_upload_record_result_layout)
    RelativeLayout mMineUploadRecordResultLayout;
    @BindView(R.id.mine_upload_record_back_rl)
    RelativeLayout mMineUploadRecordBackRl;

    protected RecyclerView mMineUploadRecordRv;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mMineUploadRecordRv = pullToRefreshRecyclerView.getRefreshableView();
        mMineUploadRecordRv.setLayoutManager(new LinearLayoutManager(
    ...
    private void setEmptyView(Boolean isEmpty) {
        ConstraintLayout empty = mBaseActivity.findViewById(R.id.item_mine_upload_record_empty_all);
        if (isEmpty) {
            empty.setVisibility(View.VISIBLE);
            mMineUploadRecordResultLayout.setVisibility(View.GONE);
        } else {
            empty.setVisibility(View.GONE);
            mMineUploadRecordResultLayout.setVisibility(View.VISIBLE);
        }
    }
     ...
     * 点击事件
     */
    @OnClick({R.id.mine_upload_record_back_rl})
    public void onViewClick(View view) {
        switch (view.getId()) {
            // 返回
            case R.id.mine_upload_record_back_rl:
                finish();
                break;
        }
    }
     ...
     * 加载界面
     */
    private void loadPage(final ArrayList<UploadRecordListItem> uploadRecordList) {
        adapter = new UploadRecordAdapter(mBaseActivity, uploadRecordList);
        adapter.setMyClickListener(new UploadRecordAdapter.MyClickListener() {
            // 选择
            @Override
            public void select(int position) {
                Intent intent = new Intent(mBaseActivity, UploadRecordDetailsActivity.class);
                intent.putExtra("ID", uploadRecordList.get(position).ID);
                intent.putExtra("status", uploadRecordList.get(position).STATE);
                startActivity(intent);
            }
        });
        mMineUploadRecordRv.setAdapter(adapter);
    }
        ...
        protected Object[] doInBackground(HashMap<String, String>... params) {
            try {
                return HttpUtils.postHttp(mBaseActivity,
                        UrlConfig.base_url + UrlConfig.upload_pay_url, params[0],
        ...
        protected void onPostExecute(Object[] result) {
            if ((boolean) result[0]) {
                try {
                    ...
                    Log.e("TAG", "汇款凭证记录列表的返回结果：" + data.toString());
                    ...
                    Gson gson = new Gson();
                    ArrayList<UploadRecordListItem> uploadRecordList = gson.fromJson(jsonArrayString,
                            new TypeToken<List<UploadRecordListItem>>() {
                            }.getType());
                    refreshComplete();
                    if (stringData != null) {
                        if (adapter == null || isRefresh) {
                            if (maxCount == 0) {
                                ...
                                setEmptyView(false);
                                loadPage(uploadRecordList);
                            }
                        } else {
                            setEmptyView(false);
                            adapter.addDataList(uploadRecordList);
                        }
                    }
                    isRefresh = false;
                } catch (Exception e) {
