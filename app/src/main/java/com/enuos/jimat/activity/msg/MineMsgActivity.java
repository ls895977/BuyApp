...
public class MineMsgActivity extends BaseActivity {
    ...
    ImageView mBack;
    @BindView(R.id.mine_msg_rv)
    PullToRefreshRecyclerView pullToRefreshRecyclerView;
    @BindView(R.id.mine_msg_result_layout)
    RelativeLayout mMineMsgResultLayout;
    @BindView(R.id.mine_msg_back_rl)
    RelativeLayout mMineMsgBackRl;

    protected RecyclerView mMineMsgRv;
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        mMineMsgRv = pullToRefreshRecyclerView.getRefreshableView();
        mMineMsgRv.setLayoutManager(new LinearLayoutManager(
    ...
    private void setEmptyView(Boolean isEmpty) {
        ConstraintLayout empty = mBaseActivity.findViewById(R.id.item_mine_msg_empty_all);
        if (isEmpty) {
            empty.setVisibility(View.VISIBLE);
            mMineMsgResultLayout.setVisibility(View.GONE);
        } else {
            empty.setVisibility(View.GONE);
            mMineMsgResultLayout.setVisibility(View.VISIBLE);
        }
    }
     ...
     * 点击事件
     */
    @OnClick({R.id.mine_msg_back_rl})
    public void onViewClick(View view) {
        switch (view.getId()) {
            // 返回
            case R.id.mine_msg_back_rl:
                finish();
                break;
        }
    }
    ...
    private void refresh() {
        ...
        params.put("messageStatus", ""); // 未读 和 已读
     ...
     * 加载界面
     */
    private void loadPage(final ArrayList<MsgListItem> msgList) {
        adapter = new MsgAdapter(mBaseActivity, msgList);
        adapter.setMyClickListener(new MsgAdapter.MyClickListener() {
            // 选择
            @Override
            public void select(int position) {
                Intent intent = new Intent(mBaseActivity, MsgDetailsActivity.class);
                intent.putExtra("ID", msgList.get(position).ID);
                startActivity(intent);
            }
        });
        mMineMsgRv.setAdapter(adapter);
    }
        ...
        protected Object[] doInBackground(HashMap<String, String>... params) {
            try {
                return HttpUtils.postHttp(mBaseActivity,
                        UrlConfig.base_url + UrlConfig.message_url, params[0],
        ...
        protected void onPostExecute(Object[] result) {
            if ((boolean) result[0]) {
                try {
                    ...
                    Log.e("TAG", "我的消息列表的返回结果：" + data.toString());
                    ...
                    Gson gson = new Gson();
                    ArrayList<MsgListItem> msgList = gson.fromJson(jsonArrayString,
                            new TypeToken<List<MsgListItem>>() {
                            }.getType());
                    refreshComplete();
                    if (stringData != null) {
