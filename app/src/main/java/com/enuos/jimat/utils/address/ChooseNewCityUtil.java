...
import com.enuos.jimat.model.CityNewBean;
import com.enuos.jimat.model.CityNewData;
...
/**
 * Created by nzz on 2017/8/24.
 */

public class ChooseNewCityUtil implements View.OnClickListener, NumberPicker.OnValueChangeListener {
    Context context;
    AlertDialog dialog;
    ChooseCityInterface cityInterface;
    NumberPicker npProvince;
//    NumberPicker npCity, npCounty;
    TextView tvCancel, tvSure;
//    String[] newCityArray = new String[3];
    String[] newCityArray = new String[1];
    CityNewBean bean;
    ...
    public void createDialog(Context context, String[] oldCityArray, ChooseCityInterface cityInterface) {
        ...
        Gson gson = new Gson();
        bean = gson.fromJson(CityNewData.getJson(), CityNewBean.class);
...
//                changeCity(i);
    ...
    /*private void changeCity(int provinceTag) {
        List<CityNewBean.Data.City> cityList = bean.getData().get(provinceTag).getCity();
    ...
    public void onValueChange(NumberPicker picker, int oldVal, int newVal) {
        switch (picker.getId()) {
            case R.id.npProvince:
                List<CityNewBean.Data> dataList = bean.getData();
...
//                newCityArray[2] = dataList.get(npProvince.getValue()).getCity().get(0).getCounty().get(0).toString();
                break;
//            case R.id.npCity:
//                List<CityNewBean.Data.City> cityList = bean.getData().get(npProvince.getValue()).getCity();
    ...
    private void setNumberPickerDividerColor(NumberPicker numberPicker) {
        ...
        for (Field pf : pickerFields) {
            if (pf.getName().equals("mSelectionDivider")) {
                ...
                try {
                    pf.set(picker, new ColorDrawable(context.getResources().getColor(R.color.btn_dark_blue)));// pf.set(picker, new Div)
