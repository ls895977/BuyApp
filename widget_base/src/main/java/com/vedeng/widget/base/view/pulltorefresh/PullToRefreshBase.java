...
import com.vedeng.widget.base.WidgetLogUtils;
import com.vedeng.widget.base.view.pulltorefresh.internal.FlipLoadingLayout;
import com.vedeng.widget.base.view.pulltorefresh.internal.LoadingLayout;
import com.vedeng.widget.base.view.pulltorefresh.internal.RotateLoadingLayout;
import com.vedeng.widget.base.view.pulltorefresh.internal.ViewCompat;

import static com.vedeng.widget.base.view.pulltorefresh.PullToRefreshBase.Mode.PULL_FROM_END;
...
public abstract class PullToRefreshBase<T extends View> extends LinearLayout implements IPullToRefresh<T> {
    ...
    protected void onReset() {
        ...
        if (mCurrentMode == PULL_FROM_END
                && null != mRefreshableView
                && mRefreshableView instanceof AbsListView) {
            ...
            if (!isReadyForPull() && oldScrollValue > 0) {
                setHeaderScroll(0);
                switch (getPullToRefreshScrollDirection()) {
                    case HORIZONTAL:
                        //Negative to check scrolling left, positive to check scrolling right.
                        if (((AbsListView) mRefreshableView).canScrollHorizontally(1)) {
                            ((AbsListView) mRefreshableView).smoothScrollBy(oldScrollValue, 0);
                        }
                        break;
                    case VERTICAL:
                    default:
                        //Negative to check scrolling up, positive to check scrolling down.
                        if (((AbsListView) mRefreshableView).canScrollVertically(1)) {
                            ((AbsListView) mRefreshableView).smoothScrollBy(oldScrollValue, 0);
                        }
                        break;
                }
    ...
    private void init(Context context, AttributeSet attrs) {
        ...
        if (a.hasValue(R.styleable.PullToRefresh_ptrRefreshableViewBackground)) {
            ...
            WidgetLogUtils.warnDeprecation("ptrAdapterViewBackground", "ptrRefreshableViewBackground");
